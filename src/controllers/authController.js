...
const { companyRegisterSchema, studentRegistrationSchema, forgotPasswordSchema, verifyResetTokenSchema, resetPasswordSchema
...
const findUserByEmail = async (email) => {
    ...
    if (student) {
        ...
        if (user) {
            return { user, userType: 'student', profileRecord: student };
        }
    }
    // Check Company
    ...
    if (company) {
        ...
        if (user) {
            return { user, userType: 'company', profileRecord: company };
        }
    }
    ...
    if (process.env.ADMIN_EMAIL?.toLowerCase() === normalizedEmail) {
        ...
        if (adminUser) {
            return { user: adminUser, userType: 'admin', profileRecord: null };
        }
    }
    return null;
};
...
exports.registerCompany = async (req, res) => {
    try {
        ...
        res.status(201).json({
            success: true,
            message: 'Registration submitted for approval. You will be able to login once an administrator approves your account.'
        });
        if (company.status === 'pending') {
            notificationService
...
const getOrCreateFreePlan = async () => {
    ...
    const updates = {};
    if (existing.max_applications !== FREE_TIER_MAX_APPLICATIONS)
        updates.max_applications = FREE_TIER_MAX_APPLICATIONS;
    if (existing.price !== 0)
        updates.price = 0;
    if (existing.billing_cycle !== 'one-time')
    ...
    if (!Array.isArray(existing.features) || existing.features.length === 0)
        updates.features = defaults.features;
    if (!existing.is_active)
        updates.is_active = true;
    if (!existing.all_zones_included)
        updates.all_zones_included = true;
