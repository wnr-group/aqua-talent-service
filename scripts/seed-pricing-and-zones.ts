...
const createFreePlan = async (zoneMap: Map<string, any>) => {
...
const createPaidPlans = async (zoneMap: Map<string, any>) => {
  console.log('\n--- Creating paid plans ---');
  ...
  for (const planData of PLANS) {
    const { data: plan, error } = await supabase
      ...
      .insert({
        ...
        priority_support: (planData as any).prioritySupport || false,
        profile_boost: (planData as any).profileBoost || false,
        application_highlight: (planData as any).applicationHighlight || false,
        is_active: true,
...
const assignJobsToRandomZones = async (zoneMap: Map<string, any>) => {
  ...
  for (const [zoneName] of zoneMap) distribution[zoneName] = 0;
  ...
  for (const job of jobs) {
    ...
    if (zoneName) distribution[zoneName] = (distribution[zoneName] || 0) + 1;
  }
...
const createFreeSubscriptionsForStudents = async (freePlan: any, zoneMap: Map<string, any>) => {
  ...
  for (const student of students) {
    ...
    const zoneRows = allZoneIds.map((zoneId) => ({ subscription_id: subscription.id, zone_id: zoneId, source: 'plan' as const }));
    if (zoneRows.length) {
