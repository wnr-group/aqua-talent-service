    ...
    try {
        ...
        for (const p of PLANS) {
            const { data: plan, error } = await supabase
                ...
                priority_support: p.prioritySupport || false,
                profile_boost: p.profileBoost || false,
                application_highlight: p.applicationHighlight || false,
                is_active: true,
        ...
        for (const a of ADDONS) {
            const { error } = await supabase.from('addons').insert({
                ...
                zone_count: a.zoneCount ?? null,
                job_credit_count: a.jobCreditCount ?? null,
                unlock_all_zones: a.unlockAllZones ?? false
            });
            if (error)
                throw error;
            console.log(`✓ ${a.name}`);
